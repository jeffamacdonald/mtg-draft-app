...
RSpec.describe TimerCalculator, type: :model do
  describe "#calculate_target_end" do
    ...
    context "when pick is on a non-holiday weekday" do
      ...
      context "when pick is in the middle between 9 and 5" do
      ...
      context "when pick is slightly before 5" do
        it "target end date time is timer minutes minus time until 5pm past 9am tomorrow" do
          # tuesday
          travel_to Time.new(2024, 05, 7, 16, 25, 00, "-07:00") do
            pick_time = Time.zone.now.in_time_zone("Pacific Time (US & Canada)")
            timer_minutes = 120
