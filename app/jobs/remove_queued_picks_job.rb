class RemoveQueuedPicksJob < ApplicationJob
  ...
  def perform(participant_pick)
    ...
    queued_picks.each do |queued_pick|
      if participant_pick.draft_participant != queued_pick.draft_participant && [1, 2].include?(queued_pick.priority_number) && queued_pick.draft_participant.queue_active?
